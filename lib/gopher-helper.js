...
class GopherHelper {
  constructor(request, response) {
    ...
    // Keeping responseJson as the single source of truth for merged object data
    this._initStoredData = function() {
      const extensionStoredData = _.get(
        this.responseJson,
        "extension.stored_data"
      );
      const taskStoredData = _.get(this.responseJson, "task.stored_data");
      if (extensionStoredData && taskStoredData) return;

      // Copy stored_data from requestJson to response object.
      _.set(
        this.responseJson,
        "extension.stored_data",
