...
module.exports = function(gopherApp) {
  ...
  // Default error handler. Overrideen with gopher.onError()
  gopherApp.setErrorHandler((err, gopher) => {
    const errorMsg = `Gopher App caught an unhandled error. Please contact the extension developer. If you are the extension developer, use gopher.setErrorHandler() to fail gracefully. View application logs for details.`;
    if (process.env.NODE_ENV !== "test") {
      console.error(err);
    }
    gopher.response.status(500);
