...
module.exports = function(gopherApp) {
  ...
  gopherApp.app.get(OAUTH_CALLBACK_ROUTE, async (req, res) => {
    ...
    try {
      ...
      return res.send(
        400,
        "There was an error fetching your access token. This is usually caused by invalid configuration values. View logs for details."
      );
    }
  });
  ...
  gopherApp.app.use(gopherUtils.notWebhook, (req, res, next) => {
    ...
    if (req.cookies[config.accessTokenName]) {
      gopherClient.setAccessToken(req.cookies[config.accessTokenName]);
      if (!gopher) gopher = {};
