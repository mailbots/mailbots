...
class WebhookHelpers {
   ...
   */
  getMailBotData(key, defaultValue) {
    if (key) {
   ...
   */
  getEmailMethod() {
    const referenceEmail = this.get("task.reference_email");
    const thisCommand = this.get("task.command");
    if (referenceEmail.to.includes(thisCommand)) {
      return "to";
    } else if (referenceEmail.cc.includes(thisCommand)) {
      return "cc";
    } else {
      return "bcc";
    }
  ...
  quickReply(message) {
    ...
    if (typeof message === "string") {
      ...
      if (!message.subject || !message.body || !message.body instanceof Array)
